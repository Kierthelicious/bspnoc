...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* Break out of .container horizontally, then center a 70vw column */
        .who-we-section {
            box-sizing: border-box;
            width: 100vw;
...
<body id="top">
    ...
    <header class="site-header" role="banner">
        <div class="site-header-inner">
            ...
            <div class="site-header-actions">
                <button type="button" class="site-header-icon-btn" id="siteSearchToggle" aria-expanded="false" aria-controls="siteSearchPanel" aria-label="Search BSP Negros Occidental website">
                    <svg viewBox="0 0 24 24" aria-hidden="true">
    ...
    <div class="site-search-panel" id="siteSearchPanel" hidden role="search">
        <div class="site-search-panel-inner">
            <p class="site-search-hint" id="siteSearchHint">Search BSP Negros Occidental — results match only content on this council page.</p>
            <div class="site-search-row">
                <label for="siteSearchInput" class="visually-hidden">Search BSP Negros Occidental</label>
                <input type="search" class="site-search-input" id="siteSearchInput" name="q" placeholder="Search BSP Negros Occidental…" autocomplete="off">
    ...
    <main class="container">
        <section class="who-we-section" id="who-we-are" aria-labelledby="who-we-are-heading">
            <div class="who-we-section-inner">
            <header class="who-we-section-head">
                <h2 id="who-we-are-heading">Who We Are</h2>
                <p class="who-we-lead">Discover the principles that shape every scout in Negros Occidental: our promise, values, and council direction.</p>
            </header>
        ...
        <details class="dropdown" id="newsroom">
            ...
            <div class="dropdown-content">
                <p>Find official advisories, announcements, and council memoranda for Negros Occidental units and partners.</p>
                <ul>
        ...
        <details class="dropdown" id="scout-shop">
            ...
            <div class="dropdown-content">
                <p>Official uniforms, badges, handbooks, and scouting essentials for units in Negros Occidental.</p>
        ...
        <details class="dropdown" id="learning-aids">
            ...
            <div class="dropdown-content">
                <p>Downloadable references and instructional materials for scouts, leaders, and training teams across the council.</p>
                <ul>
                    <li>Advancement and program guides for patrol and troop operations</li>
                    <li>Outdoor skills and safety briefing aids</li>
                    <li>Youth protection and adult volunteer onboarding summaries</li>
                    <li>Camp craft checklists and council activity templates</li>
                </ul>
            </div>
        </details>
    ...
    <script>
        ...
        let bspNegrosOccidentalSiteSearchIndex = null;
        ...
        function buildBspNegrosOccidentalSearchIndex() {
            if (bspNegrosOccidentalSiteSearchIndex !== null) {
                return bspNegrosOccidentalSiteSearchIndex;
            }
            ...
            if (heroSection) {
                const heroHeading = readableText(heroSection.querySelector("h1"));
                const heroBody = readableText(heroSection);
                entries.push({
                    title: heroHeading || "Boy Scouts of the Philippines — Negros Occidental Council",
                    snippet: readableText(heroSection.querySelector("p")).slice(0, 180) || heroBody.slice(0, 180),
                    text: heroBody.toLowerCase(),
                    href: "#"
                });
            }
            ...
            if (whoSection) {
                const titleBlock = readableText(whoSection.querySelector(".who-we-section-head"));
                const h2Who = readableText(whoSection.querySelector(".who-we-section-head h2")) || "Who We Are";

                if (titleBlock) {
                    entries.push({
                        title: `${h2Who} (overview)`,
                        snippet: readableText(whoSection.querySelector(".who-we-lead")).slice(0, 180),
                        text: titleBlock.toLowerCase(),
                        href: "#who-we-are"
                    });
                }

                whoSection.querySelectorAll(".who-we-card").forEach((cardElement) => {
                    const heading = cardElement.querySelector("h3");
                    const bodyElement = cardElement.querySelector(".who-we-card-body");
                    const titleValue = readableText(heading);
                    let blob = "";

                    if (bodyElement) {
                        blob += ` ${readableText(bodyElement)}`;
                    }

                    const templateIdentifier = cardElement.dataset.detailTemplate;
                    const templateRoot = templateIdentifier ? document.getElementById(templateIdentifier) : null;

                    if (templateRoot && templateRoot.content) {
                        const templateHolder = document.createElement("div");
                        templateHolder.appendChild(templateRoot.content.cloneNode(true));
                        blob += ` ${readableText(templateHolder)}`;
                    }

                    entries.push({
                        title: titleValue || "Council content",
                        snippet: blob.slice(0, 180).trim() || titleValue.slice(0, 180),
                        text: blob.toLowerCase(),
                        href: cardElement.id ? `#${cardElement.id}` : "#who-we-are"
                    });
                });
            }

            document.querySelectorAll("main details.dropdown[id]").forEach((detailsElement) => {
                const anchorId = detailsElement.id;

                const summaryHeading = readableText(detailsElement.querySelector("summary"));
                const dropdownBody = detailsElement.querySelector(".dropdown-content");
                const dropdownText = readableText(dropdownBody);
                const combined = `${summaryHeading} ${dropdownText}`.trim();

                entries.push({
                    title: summaryHeading || anchorId,
                    snippet: dropdownText.slice(0, 180).trim(),
                    text: combined.toLowerCase(),
                    href: `#${anchorId}`
                });
            });

            bspNegrosOccidentalSiteSearchIndex = entries;
            return entries;
        }

        function tokenizeNegrosCouncilQuery(rawQuery) {
            return rawQuery
                .trim()
                .toLowerCase()
                .split(/\s+/)
        ...
        function focusCouncilPageTarget(href) {
            if (!href || href === "#") {
                window.scrollTo({ top: 0, behavior: "smooth" });
                return;
            }

            const matched = /^#([\w.-]+)$/u.exec(href);
            ...
            if (anchorElement.tagName === "DETAILS") {
        ...
        function renderSiteSearchResults(rawQuery) {
            ...
            const queryTokens = tokenizeNegrosCouncilQuery(queryTrimmed);

            if (!queryTokens.length) {
                const hintLine = document.createElement("p");
                hintLine.className = "site-search-empty";
                hintLine.textContent = "Enter at least 2 letters to search this BSP Negros Occidental page.";
                siteSearchResults.appendChild(hintLine);
                return;
            }

            const indexItems = buildBspNegrosOccidentalSearchIndex();
            const filtered = indexItems.filter((indexEntry) =>
                queryTokens.every((token) => indexEntry.text.includes(token))
            );

            if (!filtered.length) {
                const noneLine = document.createElement("p");
                noneLine.className = "site-search-empty";
                noneLine.textContent = `No matches on this page for "${queryTrimmed}". Try keywords from headings or programs listed on this site.`;
                siteSearchResults.appendChild(noneLine);
                return;
            }

            const maxRendered = 12;
            filtered.slice(0, maxRendered).forEach((indexEntry) => {
                const hitButton = document.createElement("button");
                hitButton.type = "button";
                hitButton.className = "site-search-hit";
                hitButton.setAttribute("aria-label", `Go to ${indexEntry.title}`);

                const titleStrong = document.createElement("strong");
                titleStrong.textContent = indexEntry.title;

                const detailSpan = document.createElement("span");
                detailSpan.textContent = indexEntry.snippet;

                hitButton.append(titleStrong, detailSpan);

                hitButton.addEventListener("click", () => {
                    closeSiteSearch();
                    requestAnimationFrame(() => focusCouncilPageTarget(indexEntry.href));
                });

                siteSearchResults.appendChild(hitButton);
            });

            if (filtered.length > maxRendered) {
                const moreLine = document.createElement("p");
                moreLine.className = "site-search-empty";
                moreLine.textContent = `Showing ${maxRendered} of ${filtered.length} matches—refine your search for fewer results.`;
                siteSearchResults.appendChild(moreLine);
            }
        }
